...
ARG OPENMPI_ROOT=/opt/openmpi-${OPENMPI_VERSION}
RUN cd /tmp \
...
# install plumed
ARG PLUMED_VERSION=2.8.0
ARG PLUMED_ROOT=/opt/plumed-${PLUMED_VERSION}
RUN cd /tmp \
    && wget -O plumed-${PLUMED_VERSION}.tar.gz https://github.com/plumed/plumed2/archive/refs/tags/v${PLUMED_VERSION}.tar.gz \
    && tar -xzvf plumed-${PLUMED_VERSION}.tar.gz && cd plumed2-${PLUMED_VERSION} \
    && ./configure CXX=${OPENMPI_ROOT}/bin/mpic++ --prefix=${PLUMED_ROOT} --enable-modules=all \
    && make -j && make install \
    && rm -rf /tmp/*
ENV PATH="$PATH:${PLUMED_ROOT}/bin"
ENV LD_LIBRARY_PATH="$LD_LIBRARY_PATH:${PLUMED_ROOT}/lib"
ENV PLUMED_ROOT="${PLUMED_ROOT}/lib/plumed"
# install gromacs
ARG GROMACS_VERSION=2021.4
